...
def main():
    ...
    print("\n[5/9] libs/hbb_common/src/config.rs")
    ...
    print("\n[1/9] libs/hbb_common/src/config.rs (option2bool)")
    ...
    print("\n[8/9] flutter/lib/common.dart (option2bool)")
    ok = patch_file("flutter/lib/common.dart", [(
    ...
    # ── 9. src/server/input_service.rs — Windows 键盘输入兼容 AutoHotkey ──
    print("\n[9/9] src/server/input_service.rs (Windows AHK fix)")
    fp_ahk = "src/server/input_service.rs"
    ok_ahk = False
    if os.path.exists(fp_ahk):
        _txt = Path(fp_ahk).read_text()
        _new = _txt

        # 9a: process_chr 函数签名增加 hotkey 参数
        _old_sig = "fn process_chr(en: &mut Enigo, chr: u32, down: bool) {"
        _new_sig = "fn process_chr(en: &mut Enigo, chr: u32, down: bool, hotkey: bool) {"
        if _old_sig in _new:
            _new = _new.replace(_old_sig, _new_sig, 1)
            print("  ✅ Add hotkey parameter to process_chr")
            ok_ahk = True
        else:
            print("  ⚠️  process_chr signature not found (already patched?)")

        # 9b: 在 Linux uinput 块后插入 macOS-only key_sequence 路径
        # Windows 保持 key_down/key_up → AutoHotkey VK 热键可检测
        import re as _re_ahk
        _pat = _re_ahk.compile(
            r"    // On Wayland with uinput mode.*?\n    \}\n\n",
            _re_ahk.DOTALL
        )
        _ins = (
        ...
        _res = _pat.sub(lambda m: m.group() + "".join(_ins), _new)
        if _res != _new:
            _new = _res
            print("  ✅ Add macOS-only key_sequence path (Windows: key_down/key_up for AHK)")
            ok_ahk = True
        else:
            print("  ⚠️  Wayland block not found (already patched?)")

        if _new != _txt:
            Path(fp_ahk).write_text(_new)
            print("  ✅ input_service.rs patched")
    if ok_ahk: total += 1


    print(f"\n{'='*50}")
    print(f"🎉 Done! Applied patches to {total}/9 files.")
    if total < 9:
